...
using p_median;
namespace p_median_genetic_algorithm
{
    #region 染色体类
    public class GAChromosome : List<int>
    {
        ...
        {
            int[] index = new int[Parameter.p];
            int count = this.Count;
            int k = 0;
            for (int i = 0; i < count; i++)
            {
                if (this[i] == 1) { index[k] = i; k++; }
            }
            double fitness = 0;
            double temp;
            for (int i = 0; i < Parameter.n; i++)
            {
                ...
                {
                    temp = Math.Min(temp, Parameter.weighted_distance[i, index[j]]);
                }
                fitness = fitness + temp;
            }
            this.Fitness = 100 / fitness;
        }
