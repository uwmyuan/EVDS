    ...
    class Lagrange
    {
        ...
        private double lowerbound()
        {
            ...
            for (int i = 0; i < n; i++)
            {
                for (int j = 0; j < m; j++)
                {
                    zlk1 = zlk1 + (hidij[i, j] - lamdak[i]) * yij[i, j];
                }
                zlk2 = zlk2 + lamdak[i];
            }
            zlk = zlk1 + zlk2;
        ...
        private double upperbound()
        {
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                for (int j = 0; j < p; j++)
                {
                    temp = Math.Min(temp, hidij[i, jindex[j]]);
                }
                for (int j = 0; j < p; j++)
                {
                    if (temp == hidij[i, jindex[j]]) yij2[i, jindex[j]] = 1;
                }
                zuk = zuk + temp;
            }
            return zuk;
        ...
        private void findsolution()
        {
            ...
            for (int j = 0; j < m; j++)
            {
                for (int i = 0; i < n; i++)
                {
                    vj[j] = vj[j] + Math.Min(0, (hidij[i, j] - lamdak[i]));
                }
            }
            Sort(vj, jindex);
            for (int j = 0; j < m; j++)
            ...
            for (int j = 0; j < m; j++)
            {
                for (int i = 0; i < n; i++)
                {
                    if ((xj[j] == 1) && (hidij[i, j] - lamdak[i] < 0))
                    { yij[i, j] = 1; }
                    else { yij[i, j] = 0; }
        ...
        public void findresult() //求优的主方法
        {
            ...
            zlks.Clear();
            //计算lamda初值
            double avhidij = Average(hidij);
            for (int i = 0; i < n; i++) { lamdak[i] = (avhidij + (hi[i] - avhidij) / 2); }
            while (k <= 1000)
            {
                //求可能的最优解
                findsolution();
